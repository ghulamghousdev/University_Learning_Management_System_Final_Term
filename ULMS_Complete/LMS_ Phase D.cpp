...
void logout();
bool studentLogIn(string userName , string password , string stdNamesList[] , string stdRegNoList[]);
...
bool saveStudents(string stdRegNoList[], string stdNamesList[], string stdCourseList[][100]);
bool loadStudents(string stdRegNoList[], string stdNamesList[], string stdCourseList[][100]);
void viewSpecificStudentCourse(string stdRegNoList[], string stdNamesList[],string stdCourseList[][100] , string regNo);
//*****************************************************************************************************************************************************************
...
int main()
{
	...
	int crdHrs, semester, option, foundIndex, number;
	...
	while(userIndex==-1){
			...
			cout<<"Enter Password: ";
			cin>>password;
			studentLogIn(userName , password , stdNamesList , stdRegNoList);
		}
	while(option!=3 && option!=2){
		...
		switch(option){
			case 0:{
				...
				break;
			case 1:{
				viewSpecificStudentCourse(stdRegNoList,stdNamesList, stdCourseList,  regNo);		
				}
				break;
			case 2:{
				logout();
		 ...
		 while (option != 12 && option != 13)
		{
			...
			case 13:
			{
				...
				saveStudents(stdRegNoList, stdNamesList, stdCourseList);
				cout << "Students have been saved successfully saved" << endl;
			...
			{
				cin.ignore(15, '\n');
				cout << "Please add a input which is in range from 1 to 13.\n";
			}
			}
			continue;
		}
...
bool isValidAddCourse(string courseCodeList[], string courseCode, string courseName, int crdHrs, int semester)
{
	...
	if (courseName.size() <= 50)
	{
		...
		if (counter == 0) //To check its validity.
		{
			if (isValidCourseCode(courseCode))
			{
				if (isValidCreditHours(crdHrs))
				{
					...
					{
						cout << "Semester number is not valid. " << endl;
					}
				}
				else
				{
					cout << "Credit hours is not valid." << endl;
				}
			}
			else
			{
		...
		{
			cout << "Your course code already exists.Why are you entering the course code again & again." << endl;
		}
	}
	else
	{
		cout << "Your course name is more then of 50 characters." << endl;
	}
	return flag;
}
...
void DeleteCourse(string courseCodeList[], string courseList[], int crtHrsList[], int semList[], string courseCode)
{
	...
	if (isValid) {
		...
		cout << "Course Code is not found that you wanted to delete." << endl;
	}
}
...
void logout(){
	cout << "You have successfully log out.\nPlease log in again to get access to features." << endl;
}
void addStudent(string stdNamesList[], string stdRegNoList[], string studentName, string regNo) {
...
bool isRegNoValid(string regNo) {
	...
	if (foundCounter == 0) {
		...
		if (flag != true) {
			cout << "Your registraion number is not valid " << endl;
		}
	}
	else {
...
bool isValidStdName(string studentName) {
	...
	if (studentName.size() == counter && emptyness != counter)
	{
		...
		cout << "Student name is not valid" << endl;
	}
	return flag;
}
...
bool saveStudents(string stdRegNoList[], string stdNamesList[], string stdCourseList[][100]) {
	...
	if (outFile.is_open()) {
		for (int i = 0; i < studentIndexCounter; i++) {
			outFile << stdRegNoList[i] << "," << stdNamesList[i] << endl;
			for (int k = 0; k < arrayCounter[i]; k++) {
				outFile << stdCourseList[i][k];
				if(k+1<arrayCounter[i]){
					
					outFile<<",";
				}
				
			}
			if(i+1!=studentIndexCounter){
			outFile << endl;
			}
		}
		flag = true;
...
bool loadStudents(string stdRegNoList[], string stdNamesList[], string stdCourseList[][100]) {
	...
	if (inputFile.is_open()) {
		while (!inputFile.eof()) {
			...
			i += 1;
			for (i; i <line1.length(); i++) {
				student += line1[i];
			}
			stdRegNoList[studentIndexCounter] = reg;
			stdNamesList[studentIndexCounter] = student;
			...
			if(line2.size()>=5){
				...
				for(int k=0 ; k<comma+1 ; k++){
					store="";
					for(int m=counter ; m<counter+5 ; m++){
						store=store+line2[m];
					}
					counter=counter+6;
					if(isValidCourseCode(store)){
						stdCourseList[studentIndexCounter][k]=store;
						arrayCounter[studentIndexCounter]++;
					}
				}
			}
				studentIndexCounter++;
...
//For log in as a student

	bool studentLogIn(string userName , string password , string stdNamesList[] , string stdRegNoList[]){
		bool isValidLogIn=false;
		string name , temp;
		for(int i=0 ; i<studentIndexCounter ; i++){
			if (stdRegNoList[i] == userName ){
				foundIndex = i;
				userIndex=i;
			}
		}
		name=stdNamesList[foundIndex];
		int len=name.size();
		for(int j=len-3 ; j<len ; j++){
			temp+=name[j];
		}
		if(temp==password && userName==stdRegNoList[foundIndex]){
			cout<<"Student has been logged in successfully"<<endl;
			isValidLogIn=true;
		}
		else{
			cout<<"Log in invalid. Please Enter UserName and Password again"<<endl;
		}
		return isValidLogIn;
	}
	
	//To view courses of a specific student
	
	void viewSpecificStudentCourse(string stdRegNoList[], string stdNamesList[],string stdCourseList[][100] , string regNo){
		
			cout << left << setw(15) << "Reg No" << setw(20) << "Student Name" << setw(20) << "Course Code" << endl;
			cout << left << setw(15) << stdRegNoList[foundIndex] << setw(20) << stdNamesList[foundIndex];
			for (int k = 0; k < arrayCounter[foundIndex]; k++) {
				cout << stdCourseList[foundIndex][k];
				cout << " ";
			}
			cout<<endl;
		}		
